...
よ = record
  ...
  ; fmap-id = λ {_} → lift (fun-ext ℓ ℓ ℓ λ f →
      let
        w : ∀ x → f x ≡ Category.id (Agda (o ⊔ ℓ) ℓ ℓ) f x
        w x = refl

        z : ∀ x → comp ℂop (Category.id ℂop) (f x) ≡ f x
        z x = Category.left-id ℂop {_} {_} {f x}

        r : ∀ x → comp ℂop (Category.id ℂop) (f x) ≡ Category.id (Agda (o ⊔ ℓ) ℓ ℓ) f x
        r x = trans (z x) (w x)
      in
      fun-ext ℓ ℓ ℓ r)
  ; fmap-∘ = lift (fun-ext ℓ ℓ ℓ λ x →
               fun-ext ℓ ℓ ℓ (λ x → Eq-Category.∘-assoc ℂ))
  }
