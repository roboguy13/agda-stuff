...
-- Functor-η : ∀ {o ℓ : Level} {Src : Category o ℓ} {Tgt : Category o ℓ} →
--   ∀ {F G : Functor Src Tgt} →
--   (act-eq : Functor.act F ≡ Functor.act G) →
--   -- (Functor.fmap {o} {ℓ} {o} {ℓ} {Src} {Tgt} F ≡ subst (λ t → ∀ {A B} → (A ⇒[ Src ] B) → (t A ⇒[ Tgt ] t B)) (sym act-eq) (Functor.fmap G)) →

--   -- (prf-2 : (λ A B f → Functor.fmap F {A} {B} f) ≡ (λ A B f → subst (λ z → ∀ {A B} → z A ⇒[ Tgt ] z B) (sym act-eq) (Functor.fmap G) {A} {B} f )) →
--   (prf-2 : (λ A B f → Functor.fmap F {A} {B} f) ≡ (λ A B f → subst (λ z → ∀ A' B' → ? ⇒[ Tgt ] ?) (sym act-eq) (λ A' B' → Functor.fmap G {A} {B}) f )) →

--   -- (λ A B (f : A ⇒[ Src ] B) → Functor.fmap F {A} {B} f) ≅ (λ A B (f : A ⇒[ Src ] B) → Functor.fmap G {A} {B} f) →
--   -- (id-eq : (λ A → Functor.fmap F {A}) ≡ (λ A → subst (λ x → ∀ {A'} → x A' A' (Category.id Src) ≡ Category.id Tgt) (sym prf-2) (Functor.fmap-id G) {A})) →
--   F ≡ G
-- Functor-η = {!!}
-- Functor-η {_} {_} {Src} {Tgt} {record { act = .(Functor.act G) ; fmap = fmap ; fmap-id = fmap-id ; fmap-∘ = fmap-∘ }} {G} refl prf-2 prf-3 =
--   let
--     F : Functor Src Tgt
--     F = record { act = Functor.act G ; fmap = fmap ; fmap-id = fmap-id ; fmap-∘ = fmap-∘ }

--     fmap-id-G = Functor.fmap-id G

--     id-eq = fun-ext λ A → uip fmap-id (subst (λ x → ∀ {A'} → x A' A' (Category.id Src) ≡ Category.id Tgt) (sym prf-2) fmap-id-G {A})

--     fmap-∘-eq : (λ A B C f g → fmap-∘ {A} {B} {C} {f} {g}) ≡ (λ A B C f g → subst (λ m → ∀ {A' B' C'} {f : B' ⇒[ Src ] C'} {g : A' ⇒[ Src ] B'} → (m B' C' f ∘[ Tgt ] m A' B' g) ≡ (m A' C' (f ∘[ Src ] g))) (sym prf-2) (Functor.fmap-∘ G) {A} {B} {C} {f} {g})
--     fmap-∘-eq = fun-ext λ A → fun-ext λ B → fun-ext λ C → fun-ext λ f → fun-ext λ g → uip fmap-∘ (subst (λ m → ∀ {A' B' C'} {f : B' ⇒[ Src ] C'} {g : A' ⇒[ Src ] B'} → (m B' C' f ∘[ Tgt ] m A' B' g) ≡ (m A' C' (f ∘[ Src ] g))) (sym prf-2) (Functor.fmap-∘ G) {A} {B} {C} {f} {g})

--     F' : Functor Src Tgt
--     F' = record { act = Functor.act G ; fmap = Functor.fmap G ; fmap-id = λ {A} → Functor.fmap-id G {A}; fmap-∘ = Functor.fmap-∘ G }

--     p : F ≡ G
--     p =
--       cong₃ (λ x x₁ x₂ → record { act = Functor.act G ; fmap = λ {A} {B} f → x A B f  ; fmap-id = λ {A} → {!x₁!} ; fmap-∘ = λ {A} {B} {C} {f} {g} → {!x₂!}}) prf-2 id-eq fmap-∘-eq
--   in
--   {!!}
-- -- Functor-η {_} {_} {Src} {Tgt} {record { act = act₁ ; fmap = fmap₁ ; fmap-id = fmap-id₁ ; fmap-∘ = fmap-∘₁ }} {record { act = .act₁ ; fmap = fmap ; fmap-id = fmap-id ; fmap-∘ = fmap-∘ }} refl refl = {!!}
--   -- let
--   --   fmap-eq : (λ A B → fmap₁ {A} {B}) ≡ (λ A B → fmap {A} {B})
--   --   fmap-eq = fun-ext λ x → fun-ext λ x₁ → fun-ext λ g → prf-2 {x} {x₁} {g}
--   -- in
--   -- cong (λ x → record { act = act₁ ; fmap = x ; fmap-id = fmap-id ; fmap-∘ = fmap-∘ }) fmap-eq
--     -- (record { act = act₁ ; fmap = fmap₁ ; fmap-id = fmap-id₁ ; fmap-∘ = fmap-∘₁ })
-- -- Functor-η {F} {F} refl prf-2 | refl = {!!}
...
NatTrans-η : ∀ {o₁ ℓ₁ o₂ ℓ₂ : Level} {Src : Category o₁ ℓ₁} {Tgt : Category o₂ ℓ₂}
  ...
  -- NatTrans.natural α ≅ NatTrans.natural β →
  α ≡ β
...
_×cat_ record { Obj = Obj₁ ; _⇒_ = _⇒₁_ ; _∘_ = _∘₁_ ; id = id₁ ; left-id = left-id₁ ; right-id = right-id₁ ; ∘-assoc = ∘-assoc₁ } record { Obj = Obj ; _⇒_ = _⇒_ ; _∘_ = _∘_ ; id = id ; left-id = left-id ; right-id = right-id ; ∘-assoc = ∘-assoc } =
  record
    ...
    ; ∘-assoc = cong₂ _,_ ∘-assoc₁ ∘-assoc -- ∘-assoc₁ , ∘-assoc
    }
